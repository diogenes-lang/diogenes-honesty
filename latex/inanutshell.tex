...
and responds \atom{amount} or \atom{abort} on \code{x} (line \lineno{7}-\lineno{9}). 
The action \inlineCoco{t} models a timeout 
that covers the case of no response (line \lineno{10}).
...
\end{minted}
\end{mdframed}
and change the number \code{100} at line \lineno{19} with \code{getOrderAmount(v)}.
...
%
Therefore, we provide the annotation \code{@SkipMethod(value="<value>")}, that

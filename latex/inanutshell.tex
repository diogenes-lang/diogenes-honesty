...
which waits for a \atom{req}uest, and then answers \atom{ok} or \atom{no}.
% 
...
and cannot fulfil the contract \code{C} at session \code{x} (\code{\$0} in the output message).
...
\text{E.g.}, from the honest specification given above,
...
the \code{ContractExpiredException} is thrown by line \lineno{9} 
if the session \code{y}
is not established within a given timeout (10 sec), 
while the \code{TimeExpiredException}
is thrown by line \lineno{13} if a message is not received within the timeout.
The \code{parallel} method at lines \lineno{20}-\lineno{21} starts a new 
thread which executes the given \code{Runnable} instance.
%
...
we may want to delegate the computation of \code{amount} 
...
\item \code{HONEST}: the tool has inferred a \coco specification and verified its honesty;
\item \code{DISHONEST}: as above, but the inferred \coco specification is dishonest;
\item \code{UNKNOWN}: the tool has been unable to infer a \coco specification,

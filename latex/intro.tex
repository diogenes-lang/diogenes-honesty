...
\begin{figure}[t]
	\centering
	\includegraphics[width=0.8\textwidth]{img/primitive-schema.png}
	\caption{A schema of the primitive behaviours (from~\cite{CO2}).} \label{fig:primitives}
\end{figure}
...
The thesis is structured as follows: part I takes an overview of all the necessary concepts required to understand the contribution provided with this Master's thesis. Part II describe the Eclipse plugin and the verification strategy adopted for Java programs.

\Cref{chap:co2} shows the \textit{contract-oriented paradigm}, focusing on session-types and \coco calculus, its notions of \textit{compliance} and \textit{honesty} and presenting the contract-oriented middleware used in this work.
\Cref{chap:eclipse-pde} summarizes some aspects of the Eclipse platform and presents the two main tools we used to implement our plugin, \ie \textit{Xtext}\cite{xtext-site} and \textit{Xsemantics}\cite{xsemantics-site}. 
\Cref{chap:model-java} introduces to \textit{Java PathFinder}\cite{jpf-site}. 
